...
class ApiChatPostMessageInputContextItem(TypedDict, total=False):
...
class ApiChatPostMessageInput(TypedDict, total=False):
    ...
    context: list[ApiChatPostMessageInputContextItem]
    idempotency_key: str
    reply_to: str
    uploads: list[dict[str, Any]]


class ApiChatPostSimpleMessageInputContextItem(TypedDict, total=False):
    attributes: dict[str, Any] | None
    "Scalar key-value fields describing the context, such as route, repository, or pull-request number."
    content: str | None
    "Optional context body. The model receives it as escaped XML data, not a system instruction."
...
class ApiChatPostSimpleMessageInput(TypedDict, total=False):
    ...
    content: str
    context: list[ApiChatPostSimpleMessageInputContextItem]
    idempotency_key: str
    reply_to: str
...
class ApiChatPublishLocalToolsInputLocalToolsItemFunction(TypedDict):
    description: str
    name: str
    parameters: dict[str, Any]


class ApiChatPublishLocalToolsInputLocalToolsItem(TypedDict):
    function: ApiChatPublishLocalToolsInputLocalToolsItemFunction
    "A function implemented by the client connected to a personal thread."
    type: Literal["function"]


class ApiChatPublishLocalToolsInput(TypedDict):
    "Replace the local tools advertised by this connection for later messages it posts"

    local_tool_provider_id: str
    local_tools: list[ApiChatPublishLocalToolsInputLocalToolsItem]


class ApiChatLocalToolResultInput(TypedDict):
    ...
    provider_id: str
    request_id: str
    results: list[dict[str, Any] | dict[str, Any] | dict[str, Any]]
...
class MessageAddedPayloadMessageContextItem(TypedDict, total=False):
    attributes: dict[str, Any] | None
    "Scalar key-value fields describing the context, such as route, repository, or pull-request number."
    content: str | None
    "Optional context body. The model receives it as escaped XML data, not a system instruction."
...
class MessageAddedPayloadMessage(TypedDict, total=False):
    ...
    context: list[MessageAddedPayloadMessageContextItem] | None
...
class MessageUpdatedPayloadMessageContextItem(TypedDict, total=False):
    attributes: dict[str, Any] | None
    "Scalar key-value fields describing the context, such as route, repository, or pull-request number."
    content: str | None
    "Optional context body. The model receives it as escaped XML data, not a system instruction."
...
class MessageUpdatedPayloadMessage(TypedDict, total=False):
    ...
    content: str | None
    "Text content of the message. `null` for messages that contain only attachments."
    context: list[MessageUpdatedPayloadMessageContextItem] | None
    "Immutable structured context captured when the message was posted. Each entry has `type`, optional `title` and `content`, and scalar `attributes`. Always present; defaults to an empty array. Context is delivered to agents as escaped XML data, not as system instructions."
    created_at: str | None
    "When the message was posted (ISO 8601)."
    has_replies: bool | None
...
class LocalToolCallPayloadRequestCallsItem(TypedDict):
    arguments: dict[str, Any]
    id: str
    name: str


class LocalToolCallPayloadRequest(TypedDict):
    calls: list[LocalToolCallPayloadRequestCallsItem]
    id: str


class LocalToolCallPayload(TypedDict, total=False):
    ...
    request: LocalToolCallPayloadRequest | None
    "A correlated batch of local function calls targeted to one connection."
    thread_id: str | None


class LocalToolCancelledPayload(TypedDict, total=False):
...
class ApiChatChannel:
    ...
    @staticmethod
    def topic_org_thread(org_id: str, thread_id: str) -> str:
        return f"api:chat:org:{org_id}:thread:{thread_id}"

    # Join an organization-visibility thread by ID
    @classmethod
    async def join_org_thread(
        cls,
        socket: "Socket",
        org_id: str,
        thread_id: str,
        *,
        ...
        topic = cls.topic_org_thread(org_id, thread_id)
        channel = socket.channel(topic)
        payload: dict[str, object] = {}
        if after_cursor is not None:
            payload["after_cursor"] = after_cursor
    ...
    # Replace the local tools advertised by this connection for later messages it posts
    async def api_chat_publish_local_tools(
        self, payload: ApiChatPublishLocalToolsInput
    ) -> dict[str, Any]:
        return await self._channel.push("api:chat:publish_local_tools", payload)

    # Return one complete result batch for a connection-local tool request
    async def api_chat_local_tool_result(
        self, payload: ApiChatLocalToolResultInput
    ) -> dict[str, Any]:
        return await self._channel.push("api:chat:local_tool_result", payload)
    ...
    # Invoke local functions on the exact connection that published them
    def on_local_tool_call(
        self, callback: Callable[[LocalToolCallPayload], None]
    ) -> Callable[[], None]:
        return self._channel.on("local_tool_call", callback)

    # Stop local work because the server no longer accepts its result
    def on_local_tool_cancelled(
        self, callback: Callable[[LocalToolCancelledPayload], None]
    ) -> Callable[[], None]:
        return self._channel.on("local_tool_cancelled", callback)
