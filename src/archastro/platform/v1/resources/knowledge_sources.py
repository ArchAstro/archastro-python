...
class KnowledgeSourceSearchInput(TypedDict, total=False):
    "Search within a knowledge source"

    max_results: int | None
    "Maximum number of results to return. The platform applies its own upper bound; omit to use the default."
    ...
    mode: str | None
    'Search algorithm to use. One of `"hybrid"` (default, combines vector and full-text), `"vector"` (semantic similarity only), or `"fulltext"` (keyword matching only).'
    query: Required[str]
    "Natural language or keyword query string to search for."
    recency_days: int | None
    "When set, limits results to documents ingested within the last N days. Omit to search across all documents regardless of age."
...
class KnowledgeSourceSearchResponseDataItem(BaseModel):
    content: str | None = Field(
        default=None, description="Normalized plain-text content of the matched chunk."
    )
    content_type: str | None = Field(
        default=None, description='MIME type of the content, e.g. `"text/plain"` or `"text/html"`.'
    )
    created_at: datetime | None = Field(
        default=None, description="When this item was indexed into the knowledge base (ISO 8601)."
    )
    id: str = Field(..., description="Context item ID (`cim_...`).")
    kind: Literal["chunk"] = Field(
        default="chunk",
        description='Result variant discriminator. Always `"chunk"` for this object type.',
    )
    metadata: dict[str, Any] | None = Field(
        default=None,
        description="Arbitrary key-value metadata attached to this item by the source connector.",
    )
    raw_content: dict[str, Any] | None = Field(
        default=None,
        description="Raw content payload as stored by the source connector, before normalization.",
    )
    type: str | None = Field(
        default=None,
        description='Type identifier of the parent knowledge source (e.g. `"gmail"`, `"github_activity"`). Returns `"unknown"` when the source association is not loaded.',
    )
...
class KnowledgeSourceSearchResponse(BaseModel):
    ...
    data: list[KnowledgeSourceSearchResponseDataItem] = Field(
        ..., description="Array of matching knowledge items ordered by relevance score descending."
    )
...
class AsyncKnowledgeSourceResource:
    ...
    async def list(
        ...
        *,
        ...
        Args:
            ...
            team: Exact team IDs whose sources to include.
            thread: Exact backing thread IDs whose sources to include.
    ...
    async def ingest(self, source: str, input: KnowledgeSourceIngestInput) -> ContextIngestion:
        ...
        **Pull mode** (`pull: true`) re-triggers ingestion using the source's own configured
        ...
        Args:
            ...
            input.dedupe: When `true`, reuse the source's existing document if the pushed content is byte-identical to it, instead of creating a duplicate. The reused document keeps its chunks and embeddings, and `title`/`metadata` from this request are still applied to it. Content that differs in any way always creates a new document. Defaults to `false`, which creates a new document on every push. Push mode only not valid with `pull: true`. Check `metadata.document_reused` on the returned ingestion to see whether a document was actually reused.
    ...
    async def search(
        self, source: str, input: KnowledgeSourceSearchInput
        ...
        Args:
            source: Knowledge source ID (`ksrc_...`) to search within.
            input: Request body.
            input.max_results: Maximum number of results to return. The platform applies its own upper bound; omit to use the default.
            input.min_similarity: Cosine-similarity floor for the vector leg, 0.0-1.0. Candidates below it are discarded before ranking, so a high value trades recall for precision. Pass `0.0` to disable the floor when a missed match costs more than a weak one note that with no floor every query returns results, so an empty response can no longer be read as "no match". Omit to use the default.
            input.mode: Search algorithm to use. One of `"hybrid"` (default, combines vector and full-text), `"vector"` (semantic similarity only), or `"fulltext"` (keyword matching only).
            input.query: Natural language or keyword query string to search for.
            input.recency_days: When set, limits results to documents ingested within the last N days. Omit to search across all documents regardless of age.

        Returns:
            Successful response
        """
        return await self._http.request(
            f"/api/v1/knowledge_sources/{source}/search",
            method="POST",
            body=input,
            response_type=KnowledgeSourceSearchResponse,
        )
