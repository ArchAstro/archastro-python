...
class LeaseCreateInput(TypedDict, total=False):
    ...
    force: bool | None
    "Take the lease from a live but dead holder instead of returning a conflict. Releases the prior session's lease and claims the new one atomically. Always scoped to the task's assigned owner, same as an ordinary claim."
...
class LinkListResponseDataItem(BaseModel):
    external_scope: str = Field(..., description="External container identity.")
    object_id: str = Field(..., description="Object identity within that container.")
    object_type: str = Field(..., description="External object kind.")
...
class LinkListResponse(BaseModel):
    ...
    after_cursor: str | None = None
    before_cursor: str | None = None
    data: list[LinkListResponseDataItem]
    has_more: bool
...
class AsyncLeaseResource:
    ...
    async def list(self, task: str) -> TaskSessionLeaseSummary | None:
        """
        ...
        return await self._http.request(
            ...
            response_type=TaskSessionLeaseSummary | None,
        )
    ...
    async def create(self, task: str, input: LeaseCreateInput) -> TaskSessionLease:
        ...
        Args:
            task: Task ID (`tsk_...`).
            input: Request body.
            input.force: Take the lease from a live but dead holder instead of returning a conflict. Releases the prior session's lease and claims the new one atomically. Always scoped to the task's assigned owner, same as an ordinary claim.
...
class AsyncLinkResource:
    ...
    async def list(
        ...
        *,
        ...
        Args:
            task: Task ID (`tsk_...`).
            team: Explicit owning team for privileged calls.
            user: Explicit owning user for privileged calls.
            agent: Explicit owning agent for privileged calls.
            org: Explicit organization for privileged calls; must agree with owner.
            limit: Maximum links to return. Capped at 100.
            after_cursor: Opaque cursor returned by the previous page.
        ...
        if after_cursor is not None:
            query["after_cursor"] = after_cursor
        return await self._http.request(
            f"/api/v1/tasks/{task}/links",
            query=query,
            response_type=LinkListResponse,
        )

    async def create(self, task: str, input: LinkCreateInput) -> TaskExternalLink:
        """
        Add an external link to a task
        ...
        Returns:
            The created external link, with its identity normalized.
        """
        return await self._http.request(
            f"/api/v1/tasks/{task}/links",
            method="POST",
            body=input,
            response_type=TaskExternalLink,
        )
...
class AsyncTaskResource:
    ...
    async def replace(self, task: str, input: TaskReplaceInput) -> Task:
        ...
        Args:
            ...
            input.expected_version: Aggregate version returned by the latest task read. The update fails with `task_version_conflict` if the task changed first.
            ...
            input.status: Updated status: `open`, `in_progress`, `in_review`, or `done`.
...
class LeaseResource:
    ...
    def list(self, task: str) -> TaskSessionLeaseSummary | None:
        """
        Retrieve a task's current session lease
        Returns the authenticated assignee's viewer-safe live lease summary, or null
        when no live lease exists. Fencing and opaque session identifiers are never
...
class LinkResource:
    ...
    def create(self, task: str, input: LinkCreateInput) -> TaskExternalLink:
        """
        Add an external link to a task
        Adds an indexed identity without changing the referenced object's ownership or ACL.
        For private files, upload with POST /api/v1/files, then pass external_scope=platform,
