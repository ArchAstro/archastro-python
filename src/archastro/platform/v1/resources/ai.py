...
class StreamCreateInput(TypedDict, total=False):
    ...
    "Optional caller tags recorded on the LLM-call accounting row. Allowed keys: `task_id` (max 64 chars), `work_id` (max 64), `agent_kind` (one of `worker`, `one_off`, `subagent`, `overseer`, `task_reviewer`, `interactive`, `daemon_job`), `agent_id` (max 64), `client` (max 32). Anything else is a 400."
...
class EvaluationEvaluationsInput(TypedDict, total=False):
    "Create an evaluation"

    model: str | None
    "Evaluation model identifier. Omit to use the platform default."
    questions: Required[list[dict[str, Any] | dict[str, Any] | dict[str, Any]]]
    "Nonempty list of boolean, choice, or score questions. Each question must include `id`, `type`, and `instructions`."
    session_id: str | None
    "Optional UUID grouping this evaluation under one session in the Developers dashboard. Generated when omitted."
    state: Required[Any]
    "Shared content the model judges. A string, JSON object, or JSON array. Every question in the request sees this same state."
...
class ImageEditsInput(TypedDict, total=False):
    ...
    session_id: str | None
    "Optional session UUID for grouping provider attempts in usage records. Generated when omitted."
    size: str | None
...
class AiSessionTokenInput(TypedDict, total=False):
    "Mint a principal-bound billing session token"

    entitlement: Literal["llm_calls", "run_automation"] | None
...
class ChatModelsResponseDataItem(BaseModel):
    capabilities: list[Literal["image", "search", "temperature", "thinking"]] = Field(
        ...,
        description='Machine-readable model capabilities. `"image"` marks image-input chat, `"search"` marks built-in web search, `"temperature"` marks models that accept the temperature generation parameter, and `"thinking"` marks models with configurable reasoning. Empty when the catalog entry declares no special capabilities.',
    )
...
class EvaluationModelsResponseDataItem(BaseModel):
    capabilities: list[Literal["image", "search", "temperature", "thinking"]] = Field(
        ...,
        description='Machine-readable model capabilities. `"image"` marks image-input chat, `"search"` marks built-in web search, `"temperature"` marks models that accept the temperature generation parameter, and `"thinking"` marks models with configurable reasoning. Empty when the catalog entry declares no special capabilities.',
    )
...
class EvaluationModelsResponse(BaseModel):
    """
    Successful response
    """

    data: list[EvaluationModelsResponseDataItem] = Field(
        ...,
        description="Array of available evaluation model objects. At least one entry is always present.",
    )
...
class AiSessionTokenResponse(BaseModel):
    """
    Successful response
    """
...
class AsyncStreamResource:
    ...
    async def create(self, input: StreamCreateInput) -> AsyncIterator[StreamCreateEvent]:
        ...
        Args:
            input: Request body.
            input.attribution: Optional caller tags recorded on the LLM-call accounting row. Allowed keys: `task_id` (max 64 chars), `work_id` (max 64), `agent_kind` (one of `worker`, `one_off`, `subagent`, `overseer`, `task_reviewer`, `interactive`, `daemon_job`), `agent_id` (max 64), `client` (max 32). Anything else is a 400.
...
class AsyncEvaluationResource:
    ...
    async def evaluations(self, input: EvaluationEvaluationsInput) -> AIEvaluationResult:
        """
        Create an evaluation
        ...
        Args:
            input: Request body.
            input.model: Evaluation model identifier. Omit to use the platform default.
            input.questions: Nonempty list of boolean, choice, or score questions. Each question must include `id`, `type`, and `instructions`.
            input.session_id: Optional UUID grouping this evaluation under one session in the Developers dashboard. Generated when omitted.
            input.state: Shared content the model judges. A string, JSON object, or JSON array. Every question in the request sees this same state.

        Returns:
            Typed answers as a list of boolean, choice, or score objects, plus model and usage.
        """
        return await self._http.request(
            "/api/v1/ai/evaluation/evaluations",
            method="POST",
            body=input,
            response_type=AIEvaluationResult,
        )

    async def models(self) -> EvaluationModelsResponse:
        """
        ...
        """
        return await self._http.request(
            "/api/v1/ai/evaluation/models",
            response_type=EvaluationModelsResponse,
        )
...
class AsyncImageResource:
    ...
    async def edits(self, input: ImageEditsInput) -> AIImageResult:
        ...
        Args:
            ...
            input.session_id: Optional session UUID for grouping provider attempts in usage records. Generated when omitted.
...
class AsyncAiResource:
    def __init__(self, http: HttpClient):
        ...
        self.evaluation = AsyncEvaluationResource(http)
        self.image = AsyncImageResource(http)

    async def session_token(self, input: AiSessionTokenInput) -> AiSessionTokenResponse:
        """
        Mint a principal-bound billing session token
        ...
        """
        return await self._http.request(
            "/api/v1/ai/session_token",
            method="POST",
            body=input,
            response_type=AiSessionTokenResponse,
        )
...
class EvaluationResource:
    ...
    def evaluations(self, input: EvaluationEvaluationsInput) -> AIEvaluationResult:
        """
        Create an evaluation
        Sends a shared `state` and a list of typed questions to an evaluation
        model and returns typed answers. This is not chat completion and not
    ...
    def models(self) -> EvaluationModelsResponse:
        """
        List available evaluation models
        Returns the set of evaluation models that can be used with the evaluations
        endpoint. The list reflects models currently enabled for the platform and
...
class AiResource:
    def __init__(self, http: SyncHttpClient):
        ...
        self.evaluation = EvaluationResource(http)
        self.image = ImageResource(http)

    def session_token(self, input: AiSessionTokenInput) -> AiSessionTokenResponse:
        """
        Mint a principal-bound billing session token
