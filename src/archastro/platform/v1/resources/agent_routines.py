...
class AgentRoutineUpdateInputMessagePolicy(TypedDict, total=False):
...
class AgentRoutineUpdateInput(TypedDict, total=False):
    ...
    message_policy: AgentRoutineUpdateInputMessagePolicy | None
    ...
    user: str | None
    "Optional co-owner user ID (`usr_...`) to set on the routine. Must be supplied explicitly the caller's identity is never auto-stamped. Omit to leave the existing value unchanged; send `null` (or an empty string) to clear the current co-owner."


class AgentRoutineInvokeInputDelivery(TypedDict, total=False):
    message: str | None
    "Message ID (`msg_...`) to reply to. Required when `type` is `reply`."
    thread: str | None
    "Destination thread ID (`thr_...`). Required when `type` is `thread`."
    type: Required[Literal["none", "thread", "reply"]]
...
class AgentRoutineInvokeInput(TypedDict, total=False):
    ...
    delivery: AgentRoutineInvokeInputDelivery | None
...
class AsyncAgentRoutineRunResource:
    ...
    async def journal(
        self, run: str, *, limit: int | None = None, after_cursor: str | None = None
    ) -> RunJournalPage:
        """
        ...
        Args:
            run: Routine run ID (`arr_...`) whose journal to retrieve.
        ...
        return await self._http.request(
            f"/api/v1/agent_routines/runs/{run}/journal",
            query=query,
...
class AsyncAgentRoutineResource:
    ...
    async def list(
        self, *, agent: builtins.list[str] | None = None, event_type: str | None = None
    ) -> AgentRoutineListResponse:
        """
        List routines
        ...
        Args:
            agent: Agent IDs (`agi_...`) to filter routines by. Omit to return routines across all agents. Multiple values are OR'd.
    ...
    async def update(self, routine: str, input: AgentRoutineUpdateInput) -> AgentRoutine:
        ...
        Args:
            ...
            input.user: Optional co-owner user ID (`usr_...`) to set on the routine. Must be supplied explicitly the caller's identity is never auto-stamped. Omit to leave the existing value unchanged; send `null` (or an empty string) to clear the current co-owner.
    ...
    async def invoke(self, routine: str, input: AgentRoutineInvokeInput) -> AgentRoutineRun:
        ...
        Args:
            ...
            input: Request body.
            input.delivery: Typed final-result delivery: reply to a `message`, post to a `thread`, or `none` (the default).
